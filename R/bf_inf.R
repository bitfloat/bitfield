...
#'   that is checked for Inf values.
...
#'   defined here.
#' @examples
#' bf_inf(x = bf_tbl, test = "y")
#' @importFrom checkmate assertDataFrame assertSubset
#' @importFrom rlang env_bind
#' @export

bf_inf <- function(x, test, pos = NULL, na.val = NULL, description = NULL,
                   registry = NULL){
  ...
  if(is.null(description)){
    description <- c(paste0("{FALSE} the value in ", where, " '", test, "' is not Inf."),
                     paste0("{TRUE}  the value in ", where, " '", test, "' is Inf."))
  }
  ...
  prov <- list(wasDerivedFrom = test,
               wasGeneratedBy = c(paste0("testValue: is.infinite(", test, ")"), naProv, "encodeAsBinary: 0.0.1/0"))
