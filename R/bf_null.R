...
bf_null <- function(x, test, pos = NULL, na.val = NULL, description = NULL,
  ...
  if(is.null(description)){
    description <- c(paste0("{FALSE} the value in column '", test, "' is not NULL."),
                     paste0("{TRUE}  the value in column '", test, "' is NULL."))
  }
  ...
  prov <- list(wasDerivedFrom = test,
               wasGeneratedBy = c("testValue: is.null(", test, ")", naProv, "encodeAsBinary: 0.0.1/0"))
