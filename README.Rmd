...
Finally the bitfield needs to be harvested (note: input datasets have been stored into the environment `qb_env`). This will result in an output table (with one column that has the name `QB`).
...
Complex workflows do not only contain raster data, but a mix of tabular and raster data, for example when occurrences and/or areal data are modelled based on drivers that are available as rasters. In this case the design of `queuebee` allows to use different of its' functions in different (distinct) parts of the workflow to build one overall QB.
...
- [ ] use-cases for the paper: 1. show a complex usecase that makes use of provenance data throughout the pipeline to build QB, 2. store a distribution of values in a single pixel, 3. 
