...
The mapping functions are provided to `bf_grow()`, where the bitfield is grown. A flag is declared by calling, for example, `bf_na(x = bityield, test = "x")`, which will test whether the column `x` in the table `bityield` has `NA`-values. 
...
mention here that the above examples are 1. binary, 2. ordinal (cases that start at 0), 3. integer (that start anywhere) and 4. floating point; and explain the implications...
